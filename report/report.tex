...
I compared a bias-based baseline, user-based collaborative filtering, and SVD matrix factorization, which matches what the assignment asks for. I also trained \texttt{NMF} in Surprise as a small extra matrix factorization experiment. The final system still had to handle cold-start users, because some users in the test file do not appear in the training data.
...
\textbf{NMF (extra experiment).} Non-negative matrix factorization is another built-in option in Surprise. I used a biased variant with a small grid of regularization settings so that \texttt{RMSE} stayed comparable to \texttt{SVD}. I did \emph{not} use it for the final recommendations; it is only there to check that a second factorization model does not automatically win on my split.
...
I did not want to rely on \texttt{RMSE} alone. The task is to suggest movies, and a model can still get reasonable rating error while putting relevant items too low in the ranked list. That is why I always look at top-$10$ behavior together with \texttt{RMSE}.
...
\textbf{Limitations.} The ranking step ranks each relevant item against random unseen negatives, not against the full catalog, so the \texttt{Precision@10} and \texttt{Recall@10} numbers are mainly useful for comparing models on the same protocol. The cold-start fallback for new users is a popularity list, so it is limited by design.
...
\midrule
Baseline & 0.9389 & 0.0756 & 0.2255 & 0.4503 & 0.1761 & 0.3 \\
User-Based CF & 0.9398 & 0.0370 & 0.1137 & 0.2927 & 0.0711 & 1.6 \\
SVD & 0.9366 & 0.0638 & 0.1894 & 0.4071 & 0.1424 & 0.4 \\
NMF (extra) & 0.9363 & 0.0101 & 0.0275 & 0.0807 & 0.0212 & 0.4 \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{tabular}{lrrrr}
\toprule
Model & Coverage & Avg. popularity & Max repeat & Avg. genres \\
\midrule
Baseline & 43 & 129.4 & 46 & 10.1 \\
User-Based CF & 252 & 14.1 & 15 & 10.0 \\
SVD & 160 & 88.1 & 17 & 11.6 \\
NMF & 156 & 9.9 & 33 & 8.7 \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{figure}[H]
    ...
    \caption{List-level statistics over 50 sampled users (distinct genres counted per top-10 list from \texttt{movies.csv} tags).}
    \label{fig:rec-stats}
\end{figure}

These results help explain the model behavior more clearly. Although the baseline was strong on some ranking metrics, its recommendation lists were much more concentrated and repeated the same popular items across users. Table~\ref{tab:rec-stats} and Figure~\ref{fig:rec-stats} make this concrete: low coverage and a very high max repeat for the baseline, compared with \texttt{SVD}.

The user-based model is much more personalized, but its accuracy is clearly weaker. \texttt{SVD} sits between the two on popularity and coverage. \texttt{NMF} reached a similar \texttt{RMSE} after tuning, but its top-$10$ metrics were much worse on the same validation setup, so it did not look like a better choice for recommendations here.

In this dataset, the user-based model suffers from the data structure itself: the matrix is about \texttt{98\%} sparse and the median movie has only \texttt{3} ratings, so user-user overlap is often too limited for stable neighborhoods. I did not choose \texttt{SVD} because it won every metric: the baseline is still competitive on some top-$10$ scores. I chose it because it offered the most reasonable compromise between personalization and predictive quality on this dataset: clearly better ranking than user-based \texttt{CF}, and much less concentrated lists than the baseline, while keeping a strong \texttt{RMSE}.
...
\item \textbf{Cold-start users:} return a fallback list based on global popularity (weighted rating)
\end{itemize}
...
This dataset is challenging because it is highly sparse and strongly long-tail. Those properties make models that mainly follow global popularity look stronger than expected on some metrics, and they make user-based neighborhoods less stable.

For warm users, I did not pick \texttt{SVD} because it beat every number in Table~\ref{tab:main-results}. I picked it because it was the most reasonable compromise for me: more personalized lists than the baseline (as in Table~\ref{tab:rec-stats}), and more convincing ranking behavior than user-based \texttt{CF}, without pretending the problem is solved. The final system uses \texttt{SVD} for warm users and a weighted popularity fallback for cold-start users.
